...
scenario, in the shared `d03-networking` namespace.
...
```
                       ...
                       python-app-public:8089            python-app-alias
                                │                                │
                                ▼                                ▼
                     ┌────────────────────┐          CNAME → python-app.d03-networking
...
| [04-externalname.yaml](04-externalname.yaml) | **ExternalName** | `python-app-alias` is a CNAME to `python-app.d03-networking.svc.cluster.local` — lets other namespaces/clients reach the app through a stable alias without knowing the real Service name |
...
```bash
kubectl apply -f ../00-namespace.yaml   # creates d03-networking, if not already present
```
...
kubectl rollout status deployment/redis -n d03-networking
kubectl rollout status deployment/python-app -n d03-networking
...
```bash
kubectl get svc python-app-public -n d03-networking
# k3s ServiceLB assigns a node's IP as EXTERNAL-IP

EXTIP=$(kubectl get svc python-app-public -n d03-networking -o jsonpath='{.status.loadBalancer.ingress[0].ip}')
...
kubectl exec -n d03-networking redis-client -- getent hosts python-app-alias
# resolves via CNAME to python-app.d03-networking.svc.cluster.local -> the ClusterIP
...
kubectl exec -n d03-networking redis-client -- redis-cli -h redis -p 6379 ping
# expect: connection timed out / no reply

# allowed: traffic that originates from python-app pods reaches redis normally
kubectl exec -n d03-networking deploy/python-app -- sh -c \
